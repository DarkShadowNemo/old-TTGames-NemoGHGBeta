...
def GHG_mesh(f, filepath):
    ...
    RotBoneEntrySize1 = unpack("<I", f.read(4))[0]
    f.seek(4,1)
    PosBoneEntrySize1 = unpack("<I", f.read(4))[0]
    f.seek(8,1)
    nametableLL = unpack("<I", f.read(4))[0]
    f.seek(nametableLL,0)
    NamedtableEntrySize1,=unpack("<I", f.read(4))
            ...
            if flag2a == 0x6D:
                if vertexCount == 3:
                    ...
                    """for i in range(vertexCount-6):
                        ...
                        fbd14+=1*7
                        fcd14+=1*7
                        faces2d.append([fad10,fbd10,fcd10])
                        faces2d.append([fad11,fbd11,fcd11])
                        faces2d.append([fad12,fbd12,fcd12])
                        faces2d.append([fad13,fbd13,fcd13])
                        faces2d.append([fad14,fbd14,fcd14])"""
    ...
    #get the 5th one only and called signed integers per model at a time and put numbers floats and do tricks with them

    RestoreChunkMesh = f.read()
    ...
    bpy.context.view_layer.objects.active = objects1
    ...
    mesh4aPT2 = bpy.data.meshes.new(os.path.basename(os.path.splitext(filepath)[0]))
    mesh4aPT2.from_pydata(vertices2apt2, [], faces2apt2)
    objects4aPT2 = bpy.data.objects.new(os.path.basename(os.path.splitext(filepath)[0]), mesh4aPT2)
    collection.objects.link(objects4aPT2)

    #uv_texaPT2 = mesh4aPT2.uv_layers.new()

    #uv_layeraPT2 = mesh4aPT2.uv_layers[0].data

    uv_texaPT2 = mesh4aPT2.uv_layers.new()
    uv_layeraPT2 = mesh4aPT2.uv_layers[0].data
    vert_loopsaPT2 = {}
    for l in mesh4aPT2.loops:
        vert_loopsaPT2.setdefault(l.vertex_index, []).append(l.index)
    for i, coord in enumerate(uvs2apt2):
        for li in vert_loopsaPT2[i]:
            uv_layeraPT2[li].uv = coord
...
"""def GHG_mesh(f, filepath):
    ...
    Unknown0001 = unpack("<I", f.read(4))[0]
    Unknown0002 = unpack("<I", f.read(4))[0]
    EndMaterialEntrySize = unpack("<I", f.read(4))[0]
    NamedtableEntrySize,=unpack("<I", f.read(4))
    UnknowCount001 = unpack("<I", f.read(4))[0]
    UnknowMatrixSize001 = unpack("<I", f.read(4))[0]
    UnknowCount002 = unpack("<I", f.read(4))[0]
    UnknowMatrixSize002 = unpack("<I", f.read(4))[0]
    defaultlayerCount = unpack("<I", f.read(4))[0]
    defaultlayerEntrySize1 = unpack("<I", f.read(4))[0]
    BSA_EntrySize1 = unpack("<I", f.read(4))[0]
    for i in range(11):
        unkFloat01 = unpack("<f", f.read(4))[0]
    size001 = unpack("<I", f.read(4))[0]
    float001 = unpack("<f", f.read(4))[0]
    type001 = unpack("<I", f.read(4))[0]
    typeSize = unpack("<I", f.read(4))[0]
    if TextureCount == 0:
        f.seek(Material_EntrySize,0)
        for i in range(MaterialCount):
            matEntrySizelist1 = unpack("<I", f.read(4))[0]
            matttListt.append([matEntrySizelist1])
        f.seek(matEntrySizelist1,0)
        for i, matLL in enumerate(matttListt):
            f.seek(matLL[0],0)
            f.seek(288,1)
            MaterialFlag1 = unpack("<I", f.read(4))[0]
            f.seek(92,1)
            diffuseRed = unpack("<f", f.read(4))[0]
            diffuseGreen = unpack("<f", f.read(4))[0]
            diffuseBlue = unpack("<f", f.read(4))[0]
            f.seek(24,1)
            texID = unpack("<I", f.read(4))[0]
            matID = unpack("<I", f.read(4))[0]
            f.seek(36,1)
            m = bpy.data.materials.new("GHG Material")
            m.use_nodes=True
            m.blend_method="HASHED"
            m.node_tree.nodes["Principled BSDF"].inputs[2].default_value = 1
            m.node_tree.nodes["Principled BSDF"].inputs[12].default_value = 0
            m.node_tree.nodes["Principled BSDF"].inputs[0].default_value[0] = diffuseRed
            m.node_tree.nodes["Principled BSDF"].inputs[0].default_value[1] = diffuseGreen
            m.node_tree.nodes["Principled BSDF"].inputs[0].default_value[2] = diffuseBlue
            materials.append(m)
        ...
        while f.tell() < len(Chunk):
            ...
            if Chunks == b"\x03\x01\x00\x01":
                ...
                if flag_1 == 0x6C:
                    ...
                    uv_offset1 = unpack("<I", f.read(4))[0]
                    if uv_offset1 == 83886081:
                        f.seek(2,1)
                        uvCount = unpack("B", f.read(1))[0]
                        flag_1a = unpack("B", f.read(1))[0]
                        if flag_1a == 0x6D:
                            for i in range(uvCount):
                                ...
                                f.seek(4,1)
                                #uvs.append([uvx,-uvy])
                            vcol_offset1 = unpack("<I", f.read(4))[0]
                            if vcol_offset1 == 83886080:
                                f.seek(2,1)
                                vcol_Count = unpack("B", f.read(1))[0]
                                flag_1b = unpack("B", f.read(1))[0]
                                if flag_1b == 0x6E:
                                    for i in range(vcol_Count):
                                        v_r = unpack("B", f.read(1))[0] / 127
                                        v_g = unpack("B", f.read(1))[0] / 127
                                        v_b = unpack("B", f.read(1))[0] / 127
                                        v_a = unpack("B", f.read(1))[0] / 127
                                        #rgba.append([v_r,v_g,v_b,v_a])
                                    f.seek(32,1)
                                    matIDX = unpack("<I", f.read(4))[0]
                                    if matIDX == 1:
                                        ...
                                        for material in materials: objects1.data.materials.append(material)
                                    elif matIDX == 2:
