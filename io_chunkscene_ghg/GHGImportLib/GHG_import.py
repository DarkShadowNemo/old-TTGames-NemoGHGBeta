                        ...
                        if stripcount == 0:
                            ...
                            if offset3 == 1627553811:
                                ...
                                if offset3a == 65540:
                                    ...
                                    elif offset3b == 1627553819:
                                        offset3bbb = unpack("<I", f.read(4))[0]
                                        if offset3bbb == 4:
                                            f.seek(2,1)
                                            vertexCount3bbb = unpack("B", f.read(1))[0]//2
                                            flag3bbb = unpack("B", f.read(1))[0]
                                            if flag3bbb == 0x6C:
                                                if vertexCount3bbb == 3:
                                                    for i in range(1):
                                                        vx4_offu = unpack("<f", f.read(4))[0];vy4_offu = unpack("<f", f.read(4))[0];vz4_offu = unpack("<f", f.read(4))[0];face_offu1 = unpack("B", f.read(1))[0];f.seek(3,1);uvx4_offu1 = unpack("<f", f.read(4))[0];uvy4_offu1 = unpack("<f", f.read(4))[0];f.seek(8,1);vx4_offu_ = unpack("<f", f.read(4))[0];vy4_offu_ = unpack("<f", f.read(4))[0];vz4_offu_ = unpack("<f", f.read(4))[0];face_offu = unpack("B", f.read(1))[0];f.seek(3,1);uvx4_offu = unpack("<f", f.read(4))[0];uvy4_offu = unpack("<f", f.read(4))[0];f.seek(8,1);vx4_offuu = unpack("<f", f.read(4))[0];vy4_offuu = unpack("<f", f.read(4))[0];vz4_offuu = unpack("<f", f.read(4))[0];face_offuu1 = unpack("B", f.read(1))[0];f.seek(3,1);uvx4_offuu1 = unpack("<f", f.read(4))[0];uvy4_offuu1 = unpack("<f", f.read(4))[0];f.seek(8,1);vx4_offuu_ = unpack("<f", f.read(4))[0];vy4_offuu_ = unpack("<f", f.read(4))[0];vz4_offuu_ = unpack("<f", f.read(4))[0];face_offuu = unpack("B", f.read(1))[0];f.seek(3,1);uvx4_offuu = unpack("<f", f.read(4))[0];uvy4_offuu = unpack("<f", f.read(4))[0];f.seek(8,1);vx4_offuuu = unpack("<f", f.read(4))[0];vy4_offuuu = unpack("<f", f.read(4))[0];vz4_offuuu = unpack("<f", f.read(4))[0];face_offuuu1 = unpack("B", f.read(1))[0];f.seek(3,1);uvx4_offuuu1 = unpack("<f", f.read(4))[0];uvy4_offuuu1 = unpack("<f", f.read(4))[0];f.seek(8,1);vx4_offuuu_ = unpack("<f", f.read(4))[0];vy4_offuuu_ = unpack("<f", f.read(4))[0];vz4_offuuu_ = unpack("<f", f.read(4))[0];face_offuuu = unpack("B", f.read(1))[0];f.seek(3,1);uvx4_offuuu = unpack("<f", f.read(4))[0];uvy4_offuuu = unpack("<f", f.read(4))[0];f.seek(8,1)
                                                    offset3bbb1 = unpack("<I", f.read(4))[0]
                                                    if offset3bbb1 == 1627553835:
                                                        offset3bbb2 = unpack("<I", f.read(4))[0]
                                                        if offset3bbb2 == 8:
                                                            offset3bbb3 = unpack("<I", f.read(4))[0]
                                                            if offset3bbb3 == 1627553839:
