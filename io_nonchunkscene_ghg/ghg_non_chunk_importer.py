...
def WholeMeshGHGOne(f, vertices=[],faces=[],uvs=[],colors=[],fa=-1,fb=0,fc=1):
    chunkSize = 0
    QT = f.read()
    f.seek(0)
    for i in range(len(QT)):
        ...
        if Chunk == b"\x03\x01\x00\x01":
            ...
            for i in range(vertexCount):
                vx = unpack("<h", f.read(2))[0] / 10000.0
                vy = unpack("<h", f.read(2))[0] / 10000.0
                vz = unpack("<h", f.read(2))[0] / 10000.0
                f.seek(10,1)
                vertices.append([vx,vy,vz])
            for i in range(vertexCount-2):
                fa += 1
...
def NonParseGHG(filepath):
    f = open(filepath, "rb")
    WholeMeshGHGOne(f, vertices=[],faces=[],uvs=[],colors=[],fa=-1,fb=0,fc=1)
    f.close()
